# tqsdk-seasonal — 季节性与事件驱动期货策略库

> **仓库说明**：本仓库收录基于 [TqSdk（天勤量化）](https://www.shinnytech.com/tianqin/) 开发的中国商品期货季节性策略与事件驱动策略。策略聚焦于农产品种植/收割周期、节假日资金面效应等规律，所有代码均为完整可运行的 TqSdk 策略文件。

---

## 📌 仓库结构

```
tqsdk-seasonal/
├── strategies/                    # 策略文件目录
│   ├── 01_soybean_seasonal.py     # 大豆种植季节性策略
│   ├── 02_spring_festival_effect.py  # 春节前后资金面策略
│   ├── ...                        # 更多季节性策略
│   ├── 21_ag_multi_factor.py      # 农产品多因子截面策略
│   └── 22_cross_commodity_hedge.py   # 跨品种对冲策略
└── README.md
```

---

## 📋 策略列表

| 编号 | 策略名称 | 标的合约 | 策略方向 | 季节/事件窗口 | 技术过滤 | 文件名 | 上传日期 |
|------|---------|---------|---------|--------------|---------|--------|---------|
| 01 | 大豆种植季节性策略 | DCE.m（豆粕主力） | 空：4-6月；多：10-11月 | 南美大豆上市压力期 / 美豆新作炒作期 | MA5/MA20 金叉死叉 + RSI(14) + 量比过滤 | [01_soybean_seasonal.py](strategies/01_soybean_seasonal.py) | 2026-03-02 |
| 02 | 春节前后资金面策略 | SHFE.rb（螺纹钢主力） | 空：春节前5日；多：节后首日 | 春节前流动性收紧 / 春节后资金回流 | 日期精确判断（内置2020-2030年数据） | [02_spring_festival_effect.py](strategies/02_spring_festival_effect.py) | 2026-03-02 |
| 03 | 棉花种植季节性策略 | CZCE.CF（棉花主力） | 多：3-5月；空：9-11月 | 春季种植期 / 新棉上市期 | MA20 + 成交量 | [03_cotton_seasonal.py](strategies/03_cotton_seasonal.py) | 2026-03-11 |
| 04 | 节假日效应策略 | IF（股指期货） | 节前减仓/节后做多 | 国庆/春节/五一 | 节假日日历 | [04_holiday_effect.py](strategies/04_holiday_effect.py) | 2026-03-11 |
| 05 | 原油季节性策略 | SC（原油期货） | 多：冬季；空：夏季 | 冬季取暖需求 / 夏季出行高峰 | MA20 + RSI | [05_crude_oil_seasonal.py](strategies/05_crude_oil_seasonal.py) | 2026-03-04 |
| 06 | 螺纹钢季节性策略 | SHFE.rb（螺纹钢主力） | 多：3-5月；空：7-8月 | 旺季施工 / 淡季累库 | MA20 + 持仓量 | [06_rb_seasonal.py](strategies/06_rb_seasonal.py) | 2026-03-04 |
| 07 | PTA季节性策略 | CZCE.TA（PTA主力） | 多：3-5月/9-11月；空：6-8月 | 春秋旺季 / 夏季淡季 | MA20 + 成交量 | [07_ta_seasonal.py](strategies/07_ta_seasonal.py) | 2026-03-04 |
| 08 | 橡胶季节性策略 | RU（橡胶主力） | 多：2-4月；空：7-9月 | 东南亚产胶区停割期 / 割胶旺季 | MA20 + RSI | [08_ru_seasonal.py](strategies/08_ru_seasonal.py) | 2026-03-04 |
| 09 | 燃油季节性策略 | SHFE.bu（燃油主力） | 多：冬季；空：夏季 | 冬季取暖 / 夏季需求回落 | MA20 + 成交量 | [09_heating_oil_seasonal.py](strategies/09_heating_oil_seasonal.py) | 2026-03-04 |
| 10 | 豆粕季节性策略 | DCE.m（豆粕主力） | 空：4-6月；多：10-12月 | 南美上市 / 美豆新作 | MA20 + RSI | [10_soybean_meal_seasonal.py](strategies/10_soybean_meal_seasonal.py) | 2026-03-04 |
| 11 | 白银季节性策略 | AG（白银主力） | 多：1-2月/7-8月；空：5-6月 | 工业需求季节 / 投资需求 | MA20 + RSI | [11_gold_seasonal.py](strategies/11_gold_seasonal.py) | 2026-03-05 |
| 12 | 螺纹钢季节性策略 | SHFE.rb（螺纹钢） | 多：3-5月；空：8-9月 | 建材旺季 / 产能高峰 | MA20 + 持仓量 | [12_rb_seasonal.py](strategies/12_rb_seasonal.py) | 2026-03-05 |
| 13 | 白糖季节性策略 | CZCE.SR（白糖主力） | 多：9-11月；空：3-5月 | 新榨季缺口 / 集中上市 | MA20 + 成交量 | [13_sugar_seasonal.py](strategies/13_sugar_seasonal.py) | 2026-03-06 |
| 14 | 棕榈油季节性策略 | DCE.p（棕榈油主力） | 多：9-11月；空：3-5月 | 增产/减产周期 | MA20 + RSI | [14_palm_oil_seasonal.py](strategies/14_palm_oil_seasonal.py) | 2026-03-06 |
| 15 | 玉米季节性策略 | CZCE.C（玉米主力） | 多：3-5月；空：9-11月 | 种植期天气 / 收获期压力 | MA20 | [15_corn_seasonal.py](strategies/15_corn_seasonal.py) | 2026-03-09 |
| 16 | 菜籽油季节性策略 | CZCE.OI（菜籽油主力） | 多：4-6月；空：10-12月 | 油菜籽上市周期 | MA20 + RSI | [16_rapeseed_seasonal.py](strategies/16_rapeseed_seasonal.py) | 2026-03-09 |
| 17 | 苹果季节性策略 | CZCE.AP（苹果主力） | 多：6-8月；空：10-12月 | 夏季消费 / 收获季压力 | MA20 + 成交量 | [17_apple_seasonal.py](strategies/17_apple_seasonal.py) | 2026-03-10 |
| 18 | 铝季节性策略 | SHFE.AL（铝主力） | 多：3-5月；空：7-9月 | 旺季需求 / 淡季累库 | MA20 + 持仓量 | [18_aluminum_seasonal.py](strategies/18_aluminum_seasonal.py) | 2026-03-10 |
| 19 | 棉花季节性策略 | CZCE.CF（棉花） | 多：3-5月；空：9-11月 | 种植期 / 收获期 | MA20 + 成交量 | [19_cotton_seasonal.py](strategies/19_cotton_seasonal.py) | 2026-03-11 |
| 20 | 白银季节性策略 | AG（白银期货） | 多：1-2月/7-8月；空：5-6月 | 投资/工业需求季节 | MA20 + RSI | [20_silver_seasonal.py](strategies/20_silver_seasonal.py) | 2026-03-11 |
| 21 | 农产品多因子策略 | 豆粕/玉米/棉花/白糖 | 多因子打分 | 因子信号共振 | 动量+波动率+期限结构 | [21_ag_multi_factor.py](strategies/21_ag_multi_factor.py) | 2026-03-12 |
| 22 | 跨品种对冲策略 | 原油-PTA/铜-铝 | 均值回归 | 产业链价差偏离 | 布林带+z-score | [22_cross_commodity_hedge.py](strategies/22_cross_commodity_hedge.py) | 2026-03-12 |
| 23 | 金属板块多因子策略 | 铜/铝/锌/铅/镍 | 多因子打分 | 截面排序 | 动量+波动率+期限结构+情绪 | [23_metal_multi_factor.py](strategies/23_metal_multi_factor.py) | 2026-03-13 |
| 24 | 跨期套利策略 | 铜/铝/锌/镍/原油/橡胶 | 均值回归 | 期限价差偏离 | z-score+布林带 | [24_calendar_spread_arbitrage.py](strategies/24_calendar_spread_arbitrage.py) | 2026-03-13 |
| 25 | 能源化工多因子策略 | 原油/PTA/沥青/燃料油/甲醇/橡胶 | 多因子打分 | 截面排序 | 动量+波动率+期限结构+情绪 | [25_energy_chemical_multi_factor.py](strategies/25_energy_chemical_multi_factor.py) | 2026-03-16 |
| 26 | 统计套利策略 | 螺纹钢-热卷/铝-锌/铜-铝/焦煤-焦炭 | 均值回归 | 价差偏离 | z-score+布林带 | [26_statistical_arbitrage.py](strategies/26_statistical_arbitrage.py) | 2026-03-16 |
| 27 | 农产品多因子截面策略 | 豆粕/玉米/棉花/白糖/苹果 | 多因子打分 | 截面排序 | 动量+波动率+期限结构+季节性+流动性 | [27_agri_multi_factor.py](strategies/27_agri_multi_factor.py) | 2026-03-17 |
| 28 | 能源化工价差套利策略 | 原油-燃料油/螺纹钢-热卷/焦煤-焦炭/大豆-豆粕 | 均值回归 | 产业链价差偏离 | z-score+布林带 | [28_energy_spread_arbitrage.py](strategies/28_energy_spread_arbitrage.py) | 2026-03-17 |
| 29 | 宏观因子与情绪共振策略 | 全品种截面（能源/黑色/有色/农产品） | 多因子打分 | 宏观周期+情绪共振 | 动量+波动率+期限结构+宏观周期+情绪 | [29_macro_sentiment.py](strategies/29_macro_sentiment.py) | 2026-03-18 |
| 30 | 黑色系产业链套利策略 | 焦煤/焦炭/螺纹钢/热卷/铁矿石 | 均值回归 | 产业链价差偏离 | z-score+布林带+趋势过滤 | [30_black_chain_arbitrage.py](strategies/30_black_chain_arbitrage.py) | 2026-03-18 |

---

## 📖 策略简介

### 01 — 大豆种植季节性策略 (`01_soybean_seasonal.py`)

**策略逻辑**：
- **做空窗口（4-6月）**：南美大豆（巴西/阿根廷）集中收割上市，供给压力推低国际大豆/豆粕价格。结合 MA5 死叉 MA20 + RSI < 55 + 成交量放大信号，做空 DCE 豆粕主力（`KQ.m@DCE.m`）。
- **做多窗口（10-11月）**：美豆新作收割完成，市场炒作南美种植进度，偏多格局。结合 MA5 金叉 MA20 + RSI > 45 + 成交量放大信号，做多 DCE 豆粕主力。

**风控参数**：
- 每次开仓 1 手
- 止损 1.5% / 止盈 4.0%
- 最大持仓 15 个交易日，超时强平
- 非季节窗口期自动清仓

---

### 02 — 春节前后资金面策略 (`02_spring_festival_effect.py`)

**策略逻辑**：
- **春节前做空（节前 5 个交易日）**：企业年前集中结算、建筑业停工、贸易商清库存，螺纹钢期货承压。在节前第 5 个交易日开始做空 SHFE 螺纹钢主力（`KQ.m@SHFE.rb`），持有至节前结束平仓。
- **春节后做多（节后首个交易日）**：节后资金回流、建筑业复工补库、市场情绪转多，在节后首个交易日开多，持有 3 个交易日后平仓。

**内置春节日期数据库**：覆盖 2020–2030 年每年春节公历日期，自动判断节前/节后窗口。

**风控参数**：
- 每次开仓 1 手
- 止损 1.5% / 止盈 3.5%
- 节前空头持满 5 天或节后首日强平
- 节后多头持满 3 天平仓

---

### 21 — 农产品多因子截面策略 (`21_ag_multi_factor.py`)

**策略逻辑**：
- **多因子模型**：结合动量因子、波动率因子和期限结构因子构建综合打分系统
- **因子设计**：
  - 动量因子（20日）：动量>0 得正分，动量<0 得负分
  - 波动率因子（20日年化）：低于15%得正分，高于25%扣分
  - 期限结构因子：Contango得正分，Backwardation扣分
- **持仓选择**：做多/做空得分最高/最低的品种，最多持仓2个
- **季节过滤**：冬季（12-2月）不建议做多农产品

**风控参数**：
- 每次开仓 1 手
- 止损 2.5% / 止盈 5%
- 最多持仓 2 个品种

---

### 22 — 跨品种对冲策略 (`22_cross_commodity_hedge.py`)

**策略逻辑**：
- **产业链对冲**：基于产业链逻辑的跨品种均值回归策略
- **交易对**：
  - 原油-PTA（SC vs TA）：成本推动逻辑
  - 铜-铝（CU vs AL）：需求联动逻辑
- **信号生成**：使用布林带（20日，2倍标准差）判断价差偏离
  - z-score > 2：做空价差（预期回归）
  - z-score < -2：做多价差（预期回归）
  - z-score 回归到 ±0.5 范围内时平仓
- **产业链季节性过滤**：根据能源/化工/金属旺季调整信号强度

**风控参数**：
- 每次对冲 1 手
- 止损 2% / 止盈 3%
- 产业链旺季信号加权

---

### 23 — 金属板块多因子截面策略 (`23_metal_multi_factor.py`)

**策略逻辑**：
- **截面多因子模型**：应用于金属期货品种（铜、铝、锌、铅、镍）
- **因子设计**：
  - 动量因子（30日）：动量强度转化为Z-score得分
  - 波动率因子（30日年化）：低波动率得正分，高波动率扣分
  - 期限结构因子：Contango得正分，Backwardation扣分
  - 供需情绪因子：成交量变化 + 价格趋势强度
- **持仓选择**：做多得分最高的品种，做空得分最低的品种
- **调仓频率**：每月或每5个交易日

**风控参数**：
- 每次开仓 1 手
- 止损 2.5% / 止盈 5%
- 最多同时持有 2 个品种（一多一空）

---

### 24 — 跨期套利策略 (`24_calendar_spread_arbitrage.py`)

**策略逻辑**：
- **期限套利**：利用同一品种不同到期月份合约的价差均值回归特性
- **交易品种**：
  - 金属：铜、铝、锌、镍（SHFE）
  - 能源化工：原油、橡胶（SHFE/INE）
- **交易对**：9月合约 vs 1月合约（近月-远月）
- **信号生成**：
  - z-score > 1.8：做空价差（空近月，多远月）
  - z-score < -1.8：做多价差（多近月，空远月）
  - z-score 回归到 ±0.3 时平仓
- **风控**：z-score > 2.5 止损，最大持仓 15 天

**风控参数**：
- 每次对冲 1 手
- 止损 z-score 2.5
- 止盈 z-score 0.3
- 最大持仓 15 个交易日

---

### 25 — 能源化工多因子截面策略 (`25_energy_chemical_multi_factor.py`)

**策略逻辑**：
- **多因子模型**：应用于能源化工品种（原油、PTA、沥青、燃料油、甲醇、橡胶）
- **因子设计**：
  - 动量因子（20日）：动量强度转化为Z-score得分
  - 波动率因子（20日年化）：低波动率得正分，高波动率扣分
  - 期限结构因子：Contango得正分，Backwardation扣分
  - 供需情绪因子：成交量变化 + 价格趋势强度
- **持仓选择**：做多得分最高的品种，做空得分最低的品种
- **调仓频率**：每周或每5个交易日

**风控参数**：
- 每次开仓 1 手
- 止损 2.5% / 止盈 5%
- 最多同时持有 2 个品种（一多一空）

---

### 26 — 统计套利策略 (`26_statistical_arbitrage.py`)

**策略逻辑**：
- **统计套利**：基于历史价差均值回归的跨品种套利策略
- **交易对**：
  - 螺纹钢-热卷（建材产业链）
  - 铝-锌（有色金属）
  - 铜-铝（有色金属）
  - 焦煤-焦炭（黑色系产业链）
  - 橡胶-20号胶（橡胶制品）
- **信号生成**：
  - 相关系数 > 0.6 筛选品种对
  - z-score > 1.8：做空价差（卖高估品种，买低估品种）
  - z-score < -1.8：做多价差
  - z-score 回归到 ±0.3 范围内时平仓
- **布林带辅助**：判断极端偏离

**风控参数**：
- 每次对冲 1 手
- 止损 z-score 2.5
- 止盈 z-score 0.3
- 最大持仓 15 个交易日
- 最小相关系数 0.6

---

### 27 — 农产品多因子截面策略 (`27_agri_multi_factor.py`)

**策略逻辑**：
- **截面多因子模型**：应用于农产品期货（豆粕、玉米、棉花、白糖、苹果）
- **因子设计**：
  - 动量因子（20日）：动量强度转化为Z-score得分
  - 波动率因子（20日年化）：低波动率得正分，高波动率扣分
  - 期限结构因子：MA5-MA20 差值判断趋势方向
  - 季节性因子：基于农产品种植/收获周期的月份因子
  - 流动性因子：成交量变化率
- **持仓选择**：做多得分最高的品种，做空得分最低的品种
- **调仓频率**：每月或每5个交易日

**风控参数**：
- 每次开仓 1 手
- 止损 2.5% / 止盈 5%
- 最多同时持有 2 个品种（一多一空）

---

### 28 — 能源化工价差套利策略 (`28_energy_spread_arbitrage.py`)

**策略逻辑**：
- **产业链价差套利**：基于产业链上下游关系和替代性的跨品种套利
- **交易对**：
  - 原油-燃料油（SC vs FU）：能源产业链
  - 螺纹钢-热卷（RB vs HC）：建材产业链
  - 焦煤-焦炭（JM vs J）：黑色系产业链
  - 大豆-豆粕（A vs M）：农产品产业链
- **信号生成**：
  - z-score > 1.5：做空价差（空高价品种，买低价品种）
  - z-score < -1.5：做多价差
  - z-score 回归到 ±0.5 范围内时平仓
  - z-score > 3 或 < -3 止损
- **风控**：最大持仓 15 天，超时强平

**风控参数**：
- 每次对冲 1 手
- 止损 z-score 3.0
- 止盈 z-score 0.5
- 最大持仓 15 个交易日

---

### 29 — 宏观因子与情绪共振策略 (`29_macro_sentiment.py`)

**策略逻辑**：
- **四维因子模型**：综合宏观经济增长预期、美元周期、库存周期与市场情绪四个维度，对全市场商品期货进行截面打分
- **宏观因子**：
  - 宏观周期（基于PMI季节性规律）：扩张期→做多商品，收缩期→做空商品
  - 美元周期：美元走弱（年初）→利多商品，美元走强（秋季）→利空商品
  - 库存周期：主动补库期→利多商品，主动去库期→利空商品
  - 市场情绪：成交量变化率 + 价格动量综合情绪打分
- **截面打分**：动量因子（35%）+ 波动率因子（15%）+ 期限结构因子（15%）+ 宏观因子（20%）+ 情绪因子（15%）
- **持仓选择**：做多得分最高的品种，做空得分最低的品种，最多持仓2个
- **调仓频率**：每5个交易日

**风控参数**：
- 每次开仓 1 手
- 止损 2.5% / 止盈 5%
- 最多同时持有 2 个品种（一多一空）

---

### 30 — 黑色系产业链套利策略 (`30_black_chain_arbitrage.py`)

**策略逻辑**：
- **产业链套利**：基于黑色金属产业链逻辑（上中下游关系）的跨品种均值回归策略
- **交易对**：
  - 焦煤-焦炭（JM vs J）：焦煤 → 焦炭（成本推动逻辑）
  - 螺纹钢-热卷（RB vs HC）：建材替代品套利
  - 铁矿石-螺纹钢（I vs RB）：上游-下游产业链
  - 焦炭-螺纹钢（J vs RB）：综合产业链
- **信号生成**：
  - z-score > 1.8：做空价差（空A买B，预期回归）
  - z-score < -1.8：做多价差（买A卖B，预期回归）
  - z-score 回归到 ±0.5 范围内时平仓
  - z-score > 3 或 < -3 止损
- **趋势过滤**：60日均线判断趋势方向，趋势共振时降低信号强度
- **风控**：最大持仓 15 天，超时强平，最多同时持有 2 个套利对

**风控参数**：
- 每次对冲 1 手
- 止损 z-score 3.0
- 止盈 z-score 0.5
- 最大持仓 15 个交易日
- 最多同时持有 2 个套利对

---

## 🛠️ 环境依赖

```bash
pip install tqsdk
```

需要天勤量化账号（[免费注册](https://www.shinnytech.com/tianqin/)），在策略文件中替换 `TqAuth("用户名", "密码")`。

---

## 🚀 运行方式

```bash
# 回测模式（默认）
python strategies/01_soybean_seasonal.py

# 实盘/模拟盘：修改策略文件中 TqApi 初始化参数
# api = TqApi(TqAuth("your_username", "your_password"))
```

---

## ⚠️ 风险提示

> 本仓库策略仅供学习研究使用，不构成任何投资建议。期货交易具有较高风险，请在充分理解策略逻辑并经过严格回测验证后，方可在真实账户中谨慎使用。过去的季节性规律不代表未来必然重现。

---

## 📅 更新记录

| 日期 | 更新内容 |
|------|---------|
| 2026-03-02 | 初始化仓库，上传策略 01（大豆季节性）和策略 02（春节资金面效应） |
| 2026-03-04 | 新增策略 05-10：原油、螺纹钢、PTA、橡胶、燃油、豆粕季节性策略 |
| 2026-03-05 | 新增策略 11-12：白银、螺纹钢季节性策略 |
| 2026-03-06 | 新增策略 13-14：白糖、棕榈油季节性策略 |
| 2026-03-09 | 新增策略 15-16：玉米、菜籽油季节性策略 |
| 2026-03-10 | 新增策略 17-18：苹果、铝季节性策略 |
| 2026-03-11 | 新增策略 19-20：棉花、白银季节性策略 |
| 2026-03-12 | 新增策略 21-22：农产品多因子策略、跨品种对冲策略 |
| 2026-03-13 | 新增策略 23-24：金属板块多因子策略、跨期套利策略 |
| 2026-03-16 | 新增策略 25-26：能源化工多因子策略、统计套利策略 |
| 2026-03-18 | 新增策略 29-30：宏观因子与情绪共振策略、黑色系产业链套利策略 |

---

*由 [profedge6](https://github.com/profedge6) 维护 · 基于 TqSdk 构建*
